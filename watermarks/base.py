...
class WmMethod(ABC):
    ...
    def verify(self, net, device):  # verify as in weakness_into_strength
        ...
        target_dir = os.path.join('checkpoint', self.save_model)
        net.load_state_dict(torch.load(os.path.join(target_dir, 'best.pth')))
        net.eval()
        ...
        theta = find_tolerance(length, self.thresh)  # thresh is 0.05
        
        correct = length - false_preds
        wm_acc = 100. * correct / length
        logging.info('Watermark acc: %.3f%% (%d/%d)' % (wm_acc, correct, length))
        ...
        # raise NotImplementedError('watermark loader')
    def loader(self, shuffle=True):
        logging.info('Loading WM dataset.')
        assert len(self.trigger_set) != 0
        self.wm_loader = torch.utils.data.DataLoader(self.trigger_set, batch_size=self.wm_batch_size, shuffle=shuffle)  # , num_workers=4)
