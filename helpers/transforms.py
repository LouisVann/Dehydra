...
class EmbedBlock(object):
    def __init__(self, pos, size, val=1.):
        self.pos = pos
        self.size = size
        self.val = val
    ...
    def __call__(self, tensor):
        ...
        Returns:
            Tensor: Processed Tensor image.
        """    
        x = tensor.clone()
        H = x.shape[1]
        W = x.shape[2]
        x[:, H - (self.pos[1] + self.size // 2) : H - self.pos[1], W - (self.pos[2] + self.size) : W - self.pos[2]] = self.val
        return x
